...
pub fn find_extension<'a>(
    ...
    asn1::parse(
        ...
        |p: &mut asn1::Parser<'_>| -> Result<_, AuthError> {
            ...
            let bare = p.read_element::<asn1::Tlv>()?;
            find_extension_in_seq(bare.full_data(), oid)
        },
    )
    .map_err(|_| AuthError::CertificateParseError)
}
        ...
        |p: &mut asn1::Parser<'_>| -> Result<_, AuthError> {
            p.read_element::<asn1::Sequence>()?.parse(
                |p: &mut asn1::Parser<'_>| -> Result<_, AuthError> {
                    ...
                    // Optional [1] IMPLICIT crls (RFC 5652 5.1): parsed and ignored.
                    if p.peek_tag().is_some_and(|t| t.as_u8() == Some(0xa1)) {
                        let _crls = p.read_element::<asn1::Tlv>()?;
                    }
        ...
        p.read_element::<asn1::Sequence>()?.parse(
            |p: &mut asn1::Parser<'_>| -> Result<_, AuthError> {
                    ...
                    .is_some_and(|t| t.as_u8() == Some(TAG_CONTEXT_0))
                {
                    ...
                    // RFC 5652 5.3: messageDigest is mandatory when signedAttrs
                    // is present; without it the signature would not bind the
                    // content.
                    let md = extract_message_digest_from_attrs(attrs.data())?
                        .ok_or(AuthError::CertificateParseError)?;
                    (Some(set_der), Some(md))
                } else {
                    (None, None)
                };
    ...
    while !rest.is_empty() {
        ...
        let found = asn1::parse(
            ...
            |p: &mut asn1::Parser<'_>| -> Result<_, AuthError> {
                p.read_element::<asn1::Sequence>()?.parse(
                    |p: &mut asn1::Parser<'_>| -> Result<_, AuthError> {
                        // Consume every element before deciding: Sequence::parse
                        // rejects unconsumed bytes, so an early return on a
                        // non-reason extension would fail the whole CRL.
                        let oid = p.read_element::<asn1::ObjectIdentifier>()?;
