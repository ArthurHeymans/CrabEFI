    ...
    pub fn bulk_transfer(
        ...
        while transferred_total < data.len() {
            ...
            match self.wait_transfer_td(slot_id, ep, td) {
                ...
                Err(
                    error @ (XhciError::Timeout
                    | XhciError::StallError
                ...
                Err(e) => {
                    // An unrecovered error may leave the controller owning the
                    // TD, so retain the DMA mapping and disable this ring rather
                    // than let a later retry ring the doorbell for stale work.
                    core::mem::forget(bounce);
                    if let Some(slot) = self
                        .slots
                        .get_mut(slot_id as usize)
