...
(4096-bit maximum) always processed at full width, combined by a
...
stack locals that cannot outlive their frame. The arithmetic contains no
`unsafe` code and handles only public data (key, signature, digest), so
...
verification allocates nothing: operands live in fixed stack buffers, so
certificate-chain and signer traversal leave no retained state behind.
